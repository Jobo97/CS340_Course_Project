...
public class FollowServiceTest {
    ...
    private FollowServiceProxy followServiceSpy;
    ...
    public void setup() throws IOException, TweeterRemoteException {
        ...
        Mockito.when(mockServerFacade.getFollows(validRequest, "/getfollow")).thenReturn(successResponse);

        failureResponse = new FollowResponse("An exception occurred");
        Mockito.when(mockServerFacade.getFollows(invalidRequest, "/getfollow")).thenReturn(failureResponse);

        successCount = new FollowCountResponse(true, 10, 10);
        Mockito.when(mockServerFacade.getFollowCount(validCount, "/getfollowcount")).thenReturn(successCount);

        failureCount = new FollowCountResponse(false, 0, 0);
        Mockito.when(mockServerFacade.getFollowCount(invalidCount, "/getfollowcount")).thenReturn(failureCount);

        successFollow = new UserFollowResponse(true, true);
        Mockito.when(mockServerFacade.checkFollows(validFollow, "/checkfollow")).thenReturn(successFollow);
        Mockito.when(mockServerFacade.followStatus(validFollow, "/followstatus")).thenReturn(successFollow);

        failureFollow = new UserFollowResponse(false, false);
        Mockito.when(mockServerFacade.checkFollows(invalidFollow, "/checkfollow")).thenReturn(failureFollow);
        Mockito.when(mockServerFacade.followStatus(invalidFollow, "/followstatus")).thenReturn(failureFollow);
